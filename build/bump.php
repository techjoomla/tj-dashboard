...
$coreXmlFiles = array(
	'/src/components/com_tjdashboard/tjdashboard.xml',
	'/src/plugins/api/tjdashboard/tjdashboard.xml',
	'/src/plugins/tjdashboardrenderer/chartjs/chartjs.xml',
	'/src/plugins/tjdashboardrenderer/countbox/countbox.xml',
	'/src/plugins/tjdashboardrenderer/numbercardbox/numbercardbox.xml',
	'/src/plugins/tjdashboardrenderer/tabulator/tabulator.xml',
	'/src/pkg_tjdashboard.xml',
);
...
$directoryLoopExcludeFiles = array(
	'.gitignore',
	'/build/bump.php',
	'/plg_cb_jbolo_online/cb.online.xml',
);
...
foreach ($coreXmlFiles as $coreXmlFile)
{
	if (file_exists($rootPath . $coreXmlFile))
	{
		...
		// For ES apps
		$fileContents = preg_replace('#<created>[^<]*</created>#', '<created>' .
		$version['credate'] . '</created>', $fileContents
		);
...
foreach ($iterator as $file)
{
	if ($file->isFile())
	{
		...
		if ($continue)
		{
			...
			if (preg_match('#2009\s+-\s+[0-9]{4}\s+Techjoomla.\s+All\s+rights#', $fileContents)
				&& !preg_match('#2009\s+-\s+' . $year . '\s+Techjoomla.\s+All\s+rights#', $fileContents))
			{
				$changeCopyrightDate = true;
				$fileContents = preg_replace('#2009\s+-\s+[0-9]{4}\s+Techjoomla.\s+All\s+rights#', '2009 - ' .
